  ...
  describe("Direction") {
    describe("East") {
      it("should transform cell at A3 by 2 places to reach at C3") {
        East.transformBy(2)(Cell('A', 3)) shouldBe Cell('C', 3)
      }
    }

    describe("West") {
      it("should transform cell at B2 by 1 places to reach at A2") {
        West.transformBy(1)(Cell('B', 2)) shouldBe Cell('A', 2)
      }
    }

    describe("North") {
      it("should transform cell at B2 by 4 places to reach at B6") {
        North.transformBy(4)(Cell('B', 2)) shouldBe Cell('B', 6)
      }
    }

    describe("South") {
      it("should transform cell at D2 by 2 places to reach at D1") {
        South.transformBy(2)(Cell('D', 3)) shouldBe Cell('D', 1)
      }
    }
    ...
    describe("SouthEast") {
      it("should transform cell at A6 by 5 places to reach at F1") {
        SouthEast.transformBy(5)(Cell('A', 6)) shouldBe Cell('F', 1)
      }
    }

    describe("SouthWest") {
      it("should transform cell at A1 by 4 places to reach at A1") {
        SouthWest.transformBy(4)(Cell('E', 5)) shouldBe Cell('A', 1)
      }
    }

    describe("West") {
      it("should return invalid cell at A1 when transformed by 2 places") {
        val cell: Cell = SouthWest.transformBy(2)(Cell('A', 1))
