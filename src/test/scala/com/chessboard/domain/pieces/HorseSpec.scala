...
class HorseSpec extends AnyFunSpec with Matchers {
  ...
  describe("A Horse") {
    it("should calculate horse moves from E4 to reach F6, D6, F2, D2, G5, G3, C5, C3") {
      horse.allMovesOnBoard(Cell('E', 4), board, List(moveRestrictions)) should contain theSameElementsAs
    ...
    it("should calculate horse moves from H1 to reach G3, F2") {
      horse.allMovesOnBoard(Cell('H', 1), board, List(moveRestrictions) )should contain theSameElementsAs
        List(Cell('F',2), Cell('G', 3))
    }

    it("should calculate horse moves from H8 to reach F7, G6") {
      horse.allMovesOnBoard(Cell('H', 8), board, List(moveRestrictions)) should contain theSameElementsAs
